                ...
                else if(Block.getBlockFromItem(is.getItem()) instanceof FlowerBlock || forestryCheck)
                {
                    ...
                    if(!event.getPlayer().world.isRemote)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                BeeBarker.channel.sendTo(new PacketSpawnParticles(wolf.getEntityId(), event.getPlayer().getEntityId(), false), PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(wolf.getPosX(), wolf.getPosY(), wolf.getPosZ(), 64D, wolf.world.getDimensionKey())));
                            ...
                            {
                                BeeBarker.channel.sendTo(new PacketSpawnParticles(wolf.getEntityId(), event.getPlayer().getEntityId(), true), PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(wolf.getPosX(), wolf.getPosY(), wolf.getPosZ(), 64D, wolf.world.getDimensionKey())));
                            }
            ...
            while(ite.hasNext())
            {
                ...
                if(e.getValue() > 0)
                {
                    if(e.getValue() > 120)
                    {
                        ...
                        if(player != null)
                        {
                            BeeBarker.channel.sendTo(new PacketSpawnParticles(-1, player.getEntityId(), true), PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(player.getPosX(), player.getPosY(), player.getPosZ(), 32D, player.world.getDimensionKey())));
                        }
                    }
